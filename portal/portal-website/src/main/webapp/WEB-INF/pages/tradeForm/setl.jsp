...
<body ng-app="app">
        ...
        <!--电子对账单begin-->
	        <div class="flow_item">
				<div class="bill_box clearfix">
					...
					<form id="queryTradeForm" action="<%=basePath%>queryAndStat/txnsSetl" method="post">
	...
	function createExcelForm(){
		...
		$.ajax({
			type: "POST",
			url: "<%=basePath%>queryAndStat/txnsSetlExcelForms",
	...
	function createTxtForm(){
		...
		$.ajax({
			type: "POST",
			url: "<%=basePath%>queryAndStat/txnsSetlTxtForms",
			data :$("#queryTradeForm").serialize(),
			dataType: "json",
			success: function(json) {
				$.MessageBox(json["info"]);
	...
	function getFileName(){
		$.ajax({
			type: "POST",
			url: "<%=basePath%>queryAndStat/getFileInfo?packageName="+"setl",
			dataType: "json",
			success: function(json) {
				$('#tradeFile').html("");
				if(json["err"]==null){
	...
	function downloadFile(){
		window.location='<%=basePath%>queryAndStat/downloadFile?fileName='+$("#fileallName").val()+"&"+"packageName=setl";
	}
			...
			success : function(data){
				...
				for(var i=0,l=dataStr.length;i<l;i++){ 
					...
					output = output + '<td >'+dataStr[i]['REFUNDAMT']+'</td>';
 					...
 					output = output + '<td >'+dataStr[i]['CANCELAMT']+'</td>';
					output = output + '<td >'+dataStr[i]['SUCCNUM']+'</td>';
 					output = output + '<td >'+dataStr[i]['SUCCAMT']+'</td>';
 					output = output + '<td >'+dataStr[i]['FEES']+'</td>';
					output = output + '<td >'+dataStr[i]['SETLAMT']+'</td>';
					output = output + '</tr>';
				} 
